...
const { Customer, MyRate, CCRate, SupportTicket, Followup,CLIRate,Test, Chat, CliRatesTicker, CCRatesTicker,Inquiry,Email,PrivateRate} = require('../../model/v3/customerModel');
...
const CustomerController = {
    ...
    createCustomer: async (req, res) => {
        try {
            const { companyName, companyEmail, companyWebsite, username, userEmail } = req.body;
    
            // Find duplicates using MongoDB query
            ...
            if (existingCustomer) {
                if (existingCustomer.companyName === companyName) duplicateFields.push("companyName");
                if (existingCustomer.companyEmail === companyEmail) duplicateFields.push("companyEmail");
                if (existingCustomer.companyWebsite === companyWebsite) duplicateFields.push("companyWebsite");
                if (existingCustomer.username === username) duplicateFields.push("username");
                if (existingCustomer.userEmail === userEmail) duplicateFields.push("userEmail");
    ...
    loginCustomer: async (req, res) => {
        try {
            ...
            console.log("iam in customer login")
    ...
    getCustomerById: async (req, res) => {
        try {
            ...
            const customerId = req.params.id // Use the ID from params
            

    
            // Query to find the customer with the given customerId
            const customer = await Customer.findOne({ _id: new mongoose.Types.ObjectId(customerId) });
...
getCustomerByCustomerId: async (req, res) => {
    try {
        ...
        if (!customer) return res.status(404).json({ error: '' });
...
const MyRatesController = {
    createRate: async (req, res) => {
        try {
            console.log("mynewrate",req.body)
    ...
    updateRate: async (req, res) => {
        try {
            const updatedRate = await MyRate.findByIdAndUpdate(req.params.id, req.body, { new: true });
    ...
    deleteRate: async (req, res) => {
        try {
            ...
            if (!deletedRate) return res.status(404).json({ error: 'Rate not found' });
...
const RateController = {
    createRate: async (req, res) => {
        try {
            const newRate = new CCRate(req.body);
            await newRate.save();
            res.status(201).json(newRate);
        } catch (error) {
            res.status(400).json({ error: error.message });
    ...
    getRateById: async (req, res) => {
        ...
        try {
            console.log(req.params.id)
            const rate = await CCRate.findById(req.params.id);
            console.log(rate)
            if (!rate) return res.status(404).json({ error: 'Rate not found' });
    ...
    updateRate: async (req, res) => {
        ...
        try {
            const updatedRate = await CCRate.findByIdAndUpdate(id, req.body, { new: true });
            if (!updatedRate) return res.status(404).json({ error: 'Rate not found' });
          await CCRatesTicker.deleteMany({ rateids: id });
            res.json(updatedRate);
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    ...
    deleteRate: async (req, res) => {
        const id = req.params.id
        try {
            const deletedRate = await CCRate.findByIdAndDelete(id);
            if (!deletedRate) return res.status(404).json({ error: 'Rate not found' });
            await CCRatesTicker.deleteMany({ rateids: id });
            await MyRate.deleteMany({ rateId: id });
            res.json(deletedRate);
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
...
const CLIRateController = {
    ...
    updateRate: async (req, res) => {
        const id = req.params.id        
        try {
            const updatedRate = await CLIRate.findByIdAndUpdate(id, req.body, { new: true });
            await CliRatesTicker.deleteMany({ rateids: id });
            if (!updatedRate) return res.status(404).json({ error: 'Rate not found' });
            res.json(updatedRate);
        } catch (error) {
            res.status(400).json({ error: error.message });
...
const FollowupController = {
    ...
    updateFollowup: async (req, res) => {
        try {
            ...
            if (!updatedFollowup) return res.status(404).json({ error: 'Followup not found' });
    ...
    deleteFollowup: async (req, res) => {
        try {
            ...
            if (!deletedFollowup) return res.status(404).json({ error: 'Followup not found' });
...
getCliRatesTickerById : async (req, res) => {
    try {
        ...
        if (!cliRatesTicker) return res.status(404).json({ message: 'CliRatesTicker not found' });
...
updateCliRatesTicker : async (req, res) => {
    try {
        const updatedCliRatesTicker = await CliRatesTicker.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updatedCliRatesTicker) return res.status(404).json({ message: 'CliRatesTicker not found' });
...
deleteCliRatesTicker :async (req, res) => {
    try {
        const deletedCliRatesTicker = await CliRatesTicker.findByIdAndDelete(req.params.id);
        if (!deletedCliRatesTicker) return res.status(404).json({ message: 'CliRatesTicker not found' });
...
getCCRatesTickerById : async (req, res) => {
    try {
        ...
        if (!ccRatesTicker) return res.status(404).json({ message: 'CCRatesTicker not found' });
...
updateCCRatesTicker : async (req, res) => {
    try {
        const updatedCCRatesTicker = await CCRatesTicker.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updatedCCRatesTicker) return res.status(404).json({ message: 'CCRatesTicker not found' });
...
deleteCCRatesTicker : async (req, res) => {
    try {
        const deletedCCRatesTicker = await CCRatesTicker.findByIdAndDelete(req.params.id);
        if (!deletedCCRatesTicker) return res.status(404).json({ message: 'CCRatesTicker not found' });
...
const PrivateRateController = {
    ...
    createPrivateRate : async (req, res) => {
        try {
            ...
            res.status(201).json({ message: 'Private Rate created successfully', privateRate });
        } catch (err) {
            res.status(400).json({ message: 'Error creating Private Rate', error: err.message });
        }
    },
    ...
    getAllPrivateRates : async (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Error retrieving Private Rates', error: err.message });
        }
    },
    ...
    getPrivateRateById : async (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Error retrieving Private Rate', error: err.message });
        }
    },
    ...
    updatePrivateRate : async (req, res) => {
        try {
            const updatedRate = await PrivateRate.findByIdAndUpdate(req.params.id, req.body, { new: true });
            if (!updatedRate) {
                return res.status(404).json({ message: 'Private Rate not found' });
            }
            res.status(200).json({ message: 'Private Rate updated successfully', updatedRate });
        } catch (err) {
            res.status(400).json({ message: 'Error updating Private Rate', error: err.message });
        }
    },
    ...
    deletePrivateRate : async (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Error deleting Private Rate', error: err.message });
        }
...
const sendEmail = async (recipientEmail, userFirstname, customerId, password) => {
    const mailOptions = {
        from: '"cloudqlobe" <marketing@cloudqlobe.com>', // Sender address
        to: recipientEmail, // Recipient email
        subject: 'Welcome to CloudQlobe – Your Account Has Been Successfully Created', // Updated subject line
        html: `
...
const sendTestAccountEmail = async (recipientEmail, accountDetails, accountDetailstwo) => {
    const mailOptions = {
    ...
    try {
        const info = await transporter.sendMail(mailOptions);
        console.log('Test account configuration email sent successfully:', info.messageId);
...
const sendConfigurationEmail= async(recipientEmail, recipientName, configurations, paymentLink)=> {
...
const EmailController = {
    ...
    try {
      ...
      const email = new Email({ sender,recipient, subject, body, status: 'Sent' });
      await email.save();
      ...
      const failedEmail = new Email({ recipient, subject, body, status: 'Failed' });
  ...
  sendTestAccountDetails: async (req, res) => {
    try {
        ...
        if (!recipientEmail || !accountDetails || !accountDetailstwo) {
            return res.status(400).json({
                success: false,
                message: 'Recipient email, account details, and account detailstwo are required'
            });
        }

        // Ensure that all account details fields are present
        const requiredFields = ['companyName', 'switchIP', 'port', 'country', 'countryCode', 'prefix', 'dialedPattern', 'rate', 'billingCycle', 'quality'];
        ...
        const requiredFieldsTwo = ['countryCode', 'prefix', 'dialedPattern', 'rate', 'billingCycle'];
        ...
        const emailResult = await sendTestAccountEmail(recipientEmail, accountDetails, accountDetailstwo);
...
sendConfigurationApproval: async (req, res) => {
    try {
        const { recipientEmail, recipientName, configurations, paymentLink } = req.body;

        // Validate required fields
        if (!recipientEmail || !recipientName || !configurations || configurations.length === 0) {
            return res.status(400).json({
                success: false,
                message: 'Recipient email, recipient name, and configurations are required'
            });
        }

        // Ensure each configuration has necessary fields
        const requiredFields = ['countryCode', 'countryName', 'qualityDescription', 'rate', 'profile', 'status'];
        ...
        const emailSent = await sendConfigurationEmail(recipientEmail, recipientName, configurations, paymentLink);
