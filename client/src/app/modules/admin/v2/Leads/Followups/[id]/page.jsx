...
const FollowUpDetails = () => {
  ...
  useEffect(() => {
    const fetchFollowUp = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/v3/api/followups/${id}`);
        console.log(response);
        setFollowUp(response.data);
        setStatus(response.data.followupStatus);
        const customerResponse = await axios.get(`http://localhost:5000/v3/api/customers/${response.data.customerId}`);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await axios.put(`http://localhost:5000/v3/api/followups/${id}`, updatedFollowUpData);
