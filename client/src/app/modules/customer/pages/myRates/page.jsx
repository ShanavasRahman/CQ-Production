    ...
    const fetchRatesAndTests = async () => {
      if (customerData) {
        try {
          ...
          const testsResponse = await axios.get(`http://localhost:5000/v3/api/tests`);
          ...
          const fetchedCLIRates = await Promise.all(
            cliRates.map(async (rate) => {
              const response = await axios.get(`http://localhost:5000/v3/api/clirates/${rate.rateId}`);
          ...
          const fetchedCCRates = await Promise.all(
            ccRates.map(async (rate) => {
              const response = await axios.get(`http://localhost:5000/v3/api/rates/${rate.rateId}`);
